...
mapActivity <- function(n) {
    x <- activity_labels[n, 2]
    x
}
```
...
if (!file.exists(data_file)) {
    download.file(data_url, data_file, "curl", , mode = "wb")
}
...
# colnames(ytrain2)<-'Activity' colnames(ytest2)<-'Activity'
```
...
# colnames(xtest2)<-tidy_features[,2] colnames(xtrain2)<-tidy_features[,2]
